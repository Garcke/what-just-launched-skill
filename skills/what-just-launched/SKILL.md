...
Use `products` as the primary product discovery view. Use `product_data` and `community_feedback` as source-level evidence partitions. Treat `results` as a backward-compatible mixed ranked list.
