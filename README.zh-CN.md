...
`products` 是从 `product_data` 合并出的产品实体列表，适合作为产品发现主视图；其中 `products[].community_feedback` 会放已经匹配到该产品的反馈证据。`product_data` 放产品/App/仓库/发布平台/目录类证据，顶层 `community_feedback` 放未必能匹配到具体产品的社区讨论、评论、视频和网页反馈证据。`results` 会继续保留，作为兼容旧用法的混合排序列表。
